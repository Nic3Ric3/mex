...
<div id="view-toggle">
  <button class="view-btn active" data-view="scaffold">Scaffold</button>
  <button class="view-btn" data-view="source">Source Architecture</button>
</div>
...
<div id="legend" data-view="scaffold">
...
<div id="legend-source" style="display:none;position:fixed;bottom:40px;left:24px;z-index:100;display:none;gap:16px;font-size:11px;color:#8b949e;pointer-events:none;">
  <div class="legend-item"><div class="legend-dot" style="background:#da70d6"></div> God node</div>
  <div class="legend-item"><div class="legend-dot" style="background:#58a6ff"></div> Source file</div>
  <div class="legend-item"><div class="legend-dot" style="background:#484f58"></div> Leaf node</div>
</div>
...
let currentView = 'scaffold';
let sourceData = null;
let sourceSimulation = null;
let sourceNodeG = null;
let sourceLink = null;
let sourceLinkHover = null;
...
function switchView(view) {
  ...
  const scaffoldLegend = document.getElementById('legend');
  const sourceLegend = document.getElementById('legend-source');
  ...
  if (view === 'source') {
    ...
    if (sourceData) {
      ...
      svg.append('text')
        ...
        .attr('fill', '#8b949e')
        .attr('font-size', '14px')
        .text('Loading source architecture...');
    ...
    navSim.style.display = '';
    layoutToggle.style.display = '';
    scaffoldLegend.style.display = '';
    ...
    if (graphData) {
      ...
      updateProgressBar(graphData);
      initParticles(graphData);
    }
  }
}
...
function updateSourceStats(data) {
  ...
  document.getElementById('stat-completion').textContent = data.godNodes.length + ' god';
  document.getElementById('stat-undetermined').textContent = data.leafNodes.length + ' leaf';
}

function renderSource(data) {
  const { nodes, edges, godNodes, leafNodes } = data;
  ...
  if (nodes.length === 0) {
    ...
    svg.append('text')
      ...
      .attr('fill', '#8b949e')
      .attr('font-size', '14px')
      .text('No source files found. Run mex init first.');
    return;
  }

  const godSet = new Set(godNodes);
  const leafSet = new Set(leafNodes);
  ...
  svgTransform = d3.zoomIdentity.translate(width / 2, height / 2).scale(0.8);
  svg.call(zoom.transform, svgTransform);

  // Build D3 node/edge data
  const nodeMap = new Map();
  const d3Nodes = nodes.map(id => {
    const obj = { id, isGod: godSet.has(id), isLeaf: leafSet.has(id) };
    nodeMap.set(id, obj);
    return obj;
  });

  const d3Edges = edges.map(e => ({ source: e.from, target: e.to }));

  // Adjacency
  const adjacency = new Map();
  d3Nodes.forEach(n => adjacency.set(n.id, new Set()));
  d3Edges.forEach(e => {
    const sid = typeof e.source === 'object' ? e.source.id : e.source;
    const tid = typeof e.target === 'object' ? e.target.id : e.target;
  ...
  d3Nodes.forEach(n => { inDeg.set(n.id, 0); outDeg.set(n.id, 0); });
  edges.forEach(e => {
  ...
  // Sizing
  function srcNodeSize(d) {
    if (d.isGod) return 20 + Math.min((inDeg.get(d.id) || 0) * 2, 16);
    if (d.isLeaf) return 8;
    return 12;
  }

  function srcNodeColor(d) {
    if (d.isGod) return '#da70d6';
    if (d.isLeaf) return '#484f58';
    return '#58a6ff';
  }

  function srcGlowColor(d) {
    if (d.isGod) return 'rgba(218, 112, 214, 0.5)';
    if (d.isLeaf) return 'rgba(72, 79, 88, 0.2)';
    return 'rgba(88, 166, 255, 0.3)';
  }

  // Simulation
  sourceSimulation = d3.forceSimulation(d3Nodes)
    .force('link', d3.forceLink(d3Edges).id(d => d.id).distance(100).strength(0.3))
    .force('charge', d3.forceManyBody().strength(d => d.isGod ? -800 : -300).distanceMax(500))
    .force('center', d3.forceCenter(0, 0))
    .force('collision', d3.forceCollide().radius(d => srcNodeSize(d) + 15))
  ...
  sourceLink = linkGroup.selectAll('line')
    .data(d3Edges)
    .enter().append('line')
    .attr('stroke', '#30363d')
    .attr('stroke-width', 1)
    .attr('stroke-opacity', 0.4)
    .attr('marker-end', 'url(#src-arrow)');

  sourceLinkHover = linkGroup.selectAll('.src-link-hover')
    .data(d3Edges)
    .enter().append('line')
    .attr('class', 'src-link-hover')
    .attr('stroke', 'transparent')
    .attr('stroke-width', 10);

  // Nodes
  const nodeGroup = g.append('g');
  sourceNodeG = nodeGroup.selectAll('g')
    .data(d3Nodes)
    .enter().append('g')
    .attr('cursor', 'pointer')
    .call(d3.drag()
      .on('start', (event, d) => {
        if (!event.active) sourceSimulation.alphaTarget(0.1).restart();
        d.fx = d.x; d.fy = d.y;
      })
      .on('drag', (event, d) => { d.fx = event.x; d.fy = event.y; })
      .on('end', (event, d) => {
        if (!event.active) sourceSimulation.alphaTarget(0);
        d.fx = null; d.fy = null;
      })
    );

  // Glow
  sourceNodeG.append('circle')
    .attr('r', d => srcNodeSize(d) + 4)
    .attr('fill', d => srcGlowColor(d))
    .attr('filter', 'url(#src-glow)')
    .attr('opacity', 0.4);

  // Main circle
  sourceNodeG.append('circle')
    .attr('class', 'src-node-circle')
    .attr('r', d => srcNodeSize(d))
    .attr('fill', d => srcNodeColor(d))
    .attr('fill-opacity', d => d.isLeaf ? 0.5 : 0.85)
    .attr('stroke', d => srcNodeColor(d))
    .attr('stroke-width', 2)
    .attr('stroke-opacity', 0.8);

  // Inner highlight
  sourceNodeG.append('circle')
    .attr('r', d => srcNodeSize(d) * 0.35)
    .attr('fill', 'rgba(255,255,255,0.12)');

  // Labels — filename only
  sourceNodeG.append('text')
    .attr('dy', d => srcNodeSize(d) + 14)
    .attr('text-anchor', 'middle')
    .attr('fill', d => d.isGod ? '#e6edf3' : '#8b949e')
    .attr('font-size', d => d.isGod ? '11px' : '10px')
    .attr('font-weight', d => d.isGod ? '600' : '400')
  ...
  // Hover
  sourceNodeG.on('mouseenter', (event, d) => {
    const connected = adjacency.get(d.id) || new Set();

    sourceNodeG.transition().duration(200)
      .attr('opacity', n => (n.id === d.id || connected.has(n.id)) ? 1 : 0.12);

    sourceLink.transition().duration(200)
      .attr('stroke', e => {
        const sid = typeof e.source === 'object' ? e.source.id : e.source;
        const tid = typeof e.target === 'object' ? e.target.id : e.target;
        return (sid === d.id || tid === d.id) ? '#da70d6' : '#30363d';
      })
      .attr('stroke-opacity', e => {
        const sid = typeof e.source === 'object' ? e.source.id : e.source;
        const tid = typeof e.target === 'object' ? e.target.id : e.target;
      ...
      .attr('marker-end', e => {
        const sid = typeof e.source === 'object' ? e.source.id : e.source;
        const tid = typeof e.target === 'object' ? e.target.id : e.target;
        return (sid === d.id || tid === d.id) ? 'url(#src-arrow-hl)' : 'url(#src-arrow)';
      });

    d3.select(event.currentTarget).select('.src-node-circle')
      .transition().duration(200).attr('r', srcNodeSize(d) * 1.2);
  });

  sourceNodeG.on('mouseleave', (event, d) => {
    sourceNodeG.transition().duration(300).attr('opacity', 1);
    sourceLink.transition().duration(300)
      .attr('stroke', '#30363d')
      .attr('stroke-opacity', 0.4)
      .attr('stroke-width', 1)
      .attr('marker-end', 'url(#src-arrow)');
    ...
    d3.select(event.currentTarget).select('.src-node-circle')
      .transition().duration(300).attr('r', srcNodeSize(d));
  });

  // Click — source panel
  sourceNodeG.on('click', (event, d) => {
    event.stopPropagation();
    showSourcePanel(d, edges, inDeg, outDeg, godSet, leafSet);
  });
  ...
  sourceSimulation.on('tick', () => {
    ...
    sourceLinkHover
      .attr('x1', d => d.source.x).attr('y1', d => d.source.y)
      .attr('x2', d => d.target.x).attr('y2', d => d.target.y);
    sourceNodeG.attr('transform', d => `translate(${d.x},${d.y})`);
  });

  updateSourceStats(data);
}

function showSourcePanel(d, allEdges, inDeg, outDeg, godSet, leafSet) {
  const panel = document.getElementById('side-panel');
  document.getElementById('panel-title').textContent = d.id.split('/').pop();
  document.getElementById('panel-filename').textContent = d.id;

  // Type badge
  const badgeEl = document.getElementById('panel-badge');
  if (d.isGod) {
    badgeEl.textContent = 'god node';
    badgeEl.style.background = 'rgba(218,112,214,0.15)';
    badgeEl.style.color = '#da70d6';
  } else if (d.isLeaf) {
    badgeEl.textContent = 'leaf node';
    badgeEl.style.background = 'rgba(72,79,88,0.15)';
  ...
  // Status badge — show degree
  const statusBadgeEl = document.getElementById('panel-status-badge');
  const inD = inDeg.get(d.id) || 0;
  const outD = outDeg.get(d.id) || 0;
  statusBadgeEl.textContent = 'in:' + inD + ' out:' + outD;
  statusBadgeEl.style.background = 'rgba(139,148,158,0.15)';
  statusBadgeEl.style.color = '#8b949e';

  let html = '';

  // Imports (outgoing)
  const outEdges = allEdges.filter(e => e.from === d.id);
  if (outEdges.length > 0) {
    html += '<div class="panel-section">';
    html += '<div class="panel-section-title">Imports (' + outEdges.length + ')</div>';
    outEdges.forEach(e => {
      const isGod = godSet.has(e.to);
      html += '<div class="panel-edge">';
      html += '<div class="panel-edge-target" style="color:' + (isGod ? '#da70d6' : '#58a6ff') + '">' + escapeHtml(e.to) + '</div>';
      if (isGod) html += '<div class="panel-edge-condition">god node</div>';
      html += '</div>';
    });
    html += '</div>';
  }
  ...
  const inEdges = allEdges.filter(e => e.to === d.id);
  if (inEdges.length > 0) {
    html += '<div class="panel-section">';
    html += '<div class="panel-section-title">Imported by (' + inEdges.length + ')</div>';
    inEdges.forEach(e => {
      const isLeaf = leafSet.has(e.from);
      html += '<div class="panel-edge">';
      html += '<div class="panel-edge-target" style="color:' + (isLeaf ? '#8b949e' : '#58a6ff') + '">' + escapeHtml(e.from) + '</div>';
      if (isLeaf) html += '<div class="panel-edge-condition">leaf node</div>';
      html += '</div>';
    });
    html += '</div>';
  }
  ...
  if (outEdges.length === 0 && inEdges.length === 0) {
    html += '<div class="panel-section"><div class="panel-description" style="color:#484f58">No import relationships</div></div>';
  }

  document.getElementById('panel-tab-info').innerHTML = html;
  document.getElementById('panel-tab-content').innerHTML = '<div style="color:#484f58;padding:20px;">Source file — use your editor to view content</div>';
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path == '/api/graph':
            ...
            import subprocess
            try:
                # Find the project root (parent of scaffold dir)
                project_root = os.path.dirname(SCAFFOLD_DIR)
                result = subprocess.run(
                    ['node', os.path.join(project_root, 'dist', 'cli.js'), 'init', '--json'],
                    capture_output=True, text=True, cwd=project_root, timeout=30
                )
                if result.returncode == 0:
                    brief = json.loads(result.stdout)
                    import_graph = brief.get('importGraph') or {'godNodes': [], 'leafNodes': [], 'edges': []}
                else:
                    import_graph = {'godNodes': [], 'leafNodes': [], 'edges': []}
            except Exception:
                import_graph = {'godNodes': [], 'leafNodes': [], 'edges': []}
            ...
            for e in import_graph.get('edges', []):
                node_ids.add(e['from'])
                node_ids.add(e['to'])
            for n in import_graph.get('godNodes', []):
                node_ids.add(n)
            for n in import_graph.get('leafNodes', []):
                node_ids.add(n)

            data = {
                'nodes': sorted(node_ids),
                'edges': import_graph.get('edges', []),
                'godNodes': import_graph.get('godNodes', []),
                'leafNodes': import_graph.get('leafNodes', []),
            }
            payload = json.dumps(data).encode('utf-8')
            self.send_response(200)
            self.send_header('Content-Type', 'application/json')
